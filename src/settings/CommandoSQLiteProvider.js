...
class CommandoSQLiteProvider extends SettingsProvider {
  ...
  async init(client) {
    ...
    await this.db.run(
      "CREATE TABLE IF NOT EXISTS settings (channel TEXT PRIMARY KEY, settings TEXT)"
    );
  ...
  async loadSettings() {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        this.client.emit(
          "warn",
          `SQLiteProvider couldn't parse the settings stored for guild ${row.channel}.`
        );
        this.client.emit("error", err);
        continue;
      }

      if (settings != null) this.settings[row.channel] = settings;
    }
  }
  ...
  async set(channel, key, value) {
    if (this.settings[channel] == undefined) this.settings[channel] = {};
    ...
    await this.db.run(
      ...
      channel,
      JSON.stringify(this.settings[channel])
    );
  }
